            ...
            _data.delete("users", phone, (err) => {
              if (!err) {
                if (userChecks.length > 0) {
                  for (let i = 0; i < userChecks.length - 1; i++) {
                    _data.delete("checks", userChecks[i], (err) => {
                      if (err) {
                        callback(500, {"Error": "Could not delete a check in this user's list"});
                      } 
                    });
                  }
                  callback(200)
                ...
                _data.read("users", checkData.userPhone, (err, userData)=>{
                  if (!err && userData) {
                    const userChecks = typeof(userData.checks) == "object"
                      ...
                      ? userData.checks
                      : []
                    // Remove the check from the user object
                    ...
                    callback(500, {"Error":"Could not find the user who created the check so could not remove the check from the list on user object"})
                  }
                });
              } else {
